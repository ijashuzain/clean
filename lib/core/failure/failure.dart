...
sealed class Failure with _$Failure {
  ...
  const factory Failure.serverFailure({@Default('An unexpected server error occurred.') String message}) =
      ServerFailure;

  // Internet connection issues
  const factory Failure.networkFailure({
    @Default('No internet connection. Please check your settings.') String message,
  }) = NetworkFailure;

  // Local database issues (Hive/SQFlite)
  const factory Failure.cacheFailure({@Default('Could not access local data.') String message}) = CacheFailure;
